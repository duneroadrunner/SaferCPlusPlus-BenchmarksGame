...
template <bool modei>
void EvalPart(double *RESTRICT src, double *RESTRICT dst,
...
#define MAX_N 10000
	ALIGNAS(16) std::array<double, MAX_N> u;
	ALIGNAS(16) std::array<double, MAX_N> v, tmp;
...
int main(int argc, char *argv[]) {
	...
	if (MAX_N < N) {
		printf("Parameter values larger than %d are not supported. \n", MAX_N);
		return -1;
	}
	printf("%.9f\n", spectral_game(N));
	return 0;
