...
int main(int argc, char* argv[])
{
	...
	const int smax_x = mse::CInt(max_x);
#pragma omp parallel for
	...
	const int sheight = mse::CInt(height);
#pragma omp parallel for schedule(guided)
	for (int sy = 0; sy < sheight; ++sy)
	{
		...
		for (mse::msev_size_t x = 0; x < max_x; ++x)
		{
			...
			//cr0_x += 8 * x;
			mse::mstd::array<double, 8> cr0_x{ cr0[8 * x], cr0[8 * x + 1], cr0[8 * x + 2], cr0[8 * x + 3], cr0[8 * x + 4], cr0[8 * x + 5], cr0[8 * x + 6], cr0[8 * x + 7] };
			mse::mstd::array<double, 8> cr;
			mse::mstd::array<double, 8> ci;
